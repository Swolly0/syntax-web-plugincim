...
public class JBFreezeZaman : BasePlugin, IPluginConfig<Config>
{
    public override string ModuleName { get; } = "Freeze Zaman";
    public override string ModuleVersion { get; } = "1.0.0";
    public override string ModuleDescription { get; } = "JB | !fz komutu ile freeze geri sayimi baslatilabilir.";
    ...
    public void OnConfigParsed(Config config)
    {
        ...
        {
            Console.WriteLine($"[Freeze Zaman] You are using an old configuration file. Version you are using:{config.Version} - New Version: {ModuleConfigVersion}");
        }
    ...
    public override void Load(bool hotReload)
    {
        ...
        {
			AddCommand("css_fz", "Freeze Zaman", (player, command) => FreezeZaman(player, command));
			AddCommand("css_freezezaman", "Freeze Zaman", (player, command) => FreezeZaman(player, command));
			
			AddCommand("css_fz0", "Freeze Zaman Iptal", (player, command) => FreezeZaman0(player, command));
			AddCommand("css_freezezaman0", "Freeze Zaman Iptal", (player, command) => FreezeZaman0(player, command));		
	...
	public void FreezeZaman(CCSPlayerController? player, CommandInfo info)
    {	
	...
	public void FreezeZaman0(CCSPlayerController? player, CommandInfo info)
    {	
		if(player != null && NativeAPI.GetMapName().Contains("jb_")){
			if (AdminManager.PlayerHasPermissions(player, "@css/generic") || player.is_ct()){
