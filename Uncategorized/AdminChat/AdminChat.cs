...
public class AdminChat : BasePlugin
{
    ...
    public override string ModuleAuthor => "Swolly";
    ...
    public void OnCommandAsay(CCSPlayerController? player, CommandInfo command)
    {
        if (player is null)
        {
            ...
            {
                Console.WriteLine("Komut kullanýmý: css_adminsay <mesaj>");
                return;
            }
            ...
            foreach (CCSPlayerController currentPlayer in Utilities.GetPlayers())
            {
                ...
                currentPlayer.PrintToChat($" \x0b[www.plugincim.com]\x01 Console\x04: {GetArgs(command, 1)}");
            }
            return;
        }
        ...
        if (command.ArgCount < 2)
        {
            player.PrintToChat("Komut kullanýmý: css_adminsay <mesaj>");
            return;
        }
        ...
        foreach (CCSPlayerController currentPlayer in players)
        {
            ...
            {
                currentPlayer.PrintToChat($" \x0b[www.plugincim.com] \x01{player.PlayerName} \x04: {GetArgs(command, 1)}");
            }
        }
        Console.WriteLine($" \x0b[www.plugincim.com] \x01{player.PlayerName}\x04: {GetArgs(command, 1)}");
    }
    ...
    public void OnCommandTAsay(CCSPlayerController? player, CommandInfo command)
    {
        ...
        {
            player.PrintToChat($" \x0b[www.plugincim.com] admin chat açýldý.");
            g_btoggleasay[player.Slot] = true;

        }
        else
        {
            player.PrintToChat($" \x0b[www.plugincim.com] admin chat kapatýldý.");
    ...
    private HookResult OnPlayerChat(CCSPlayerController? player, CommandInfo info)
    {
        if (player is null)
        {
            foreach (CCSPlayerController currentPlayer in Utilities.GetPlayers())
            {
                ...
                currentPlayer.PrintToChat($" \x0b[www.plugincim.com]\x01 Console\x04: {GetArgs(info, 1)}");
            }
        ...
        else
        {
            ...
            foreach (CCSPlayerController currentPlayer in players)
            {
                ...
                currentPlayer.PrintToChat($" \x0b[www.plugincim.com] \x01{player.PlayerName}\x04: {GetArgs(info, 1)}");
            }
            Console.WriteLine($" \x0b[www.plugincim.com] \x01{player.PlayerName}\x04: {GetArgs(info, 1)}");
            return HookResult.Handled;
        }
    }
