...
public class Config : IBasePluginConfig
{
    ...
    public string? t_model { get; set; } = "characters/models/tm_jungle_raider/tm_jungle_raider_variantc.vmdl";

    [JsonPropertyName("ct_model")]
    public string? ct_model { get; set; } = "characters/models/tm_professional/tm_professional_varf5.vmdl";
}

public class Model0 : BasePlugin, IPluginConfig<Config>
{
    public override string ModuleName { get; } = "Model0";
    public override string ModuleVersion { get; } = "1.0.0";
    public override string ModuleDescription { get; } = "!model0 komutu ile oyuncularin modelleri degistirilir.";
    ...
    public void OnConfigParsed(Config config)
    {
        ...
        {
            Console.WriteLine($"[Model0] You are using an old configuration file. Version you are using:{config.Version} - New Version: {ModuleConfigVersion}");
        }
    ...
    public override void Load(bool hotReload)
    {
        ...
        {
            AddCommand("model0", "Oyuncu(larin) modelini degistir.", (player, command) => ModelReset(player, command));
        }
    }

    [RequiresPermissions("@css/generic")]
    public void ModelReset(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (AdminManager.PlayerHasPermissions(player, "@css/generic"))
        {
            ...
            if (NativeAPI.GetMapName().Contains("jb_"))
            {
                foreach (var p in Utilities.GetPlayers())
                    if (p != null && p.IsValid)
                        if (p.TeamNum == 2 && !string.IsNullOrEmpty(Config.t_model))
                            p.PlayerPawn.Value!.SetModel(@"" + Config.t_model);
            }
            else
            {
                foreach (var p in Utilities.GetPlayers())
                    if (p != null && p.IsValid)
                        ...
                        if (p.TeamNum == 3 && !string.IsNullOrEmpty(Config.ct_model))
                            p.PlayerPawn.Value!.SetModel(@"" + Config.ct_model);
            }
        }
        else
        {
